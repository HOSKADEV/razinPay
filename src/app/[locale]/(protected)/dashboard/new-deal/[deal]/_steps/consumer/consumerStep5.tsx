...
export const ConsumerStep5 = ({deal}:{deal:Deal}) => {
    ...
    return (
      <div className="space-y-16 my-10">
          ...
          <div className="w-full rounded-md border border-primary flex justify-between items-center p-10">
              <p>
                {t("home.start-deal.consumer.step-5.hint")} 
              </p>
              <div>
              <Button className="w-full" disabled={deal.status == dealStatus.CLOSED} onClick={() => {
                updateDealStatus({dealId:deal.id, newStatus:dealStatus.CLOSED})
                router.refresh()
              }}>
                {t("home.start-deal.consumer.step-5.confirm-btn")} 
              </Button>
              </div>
          </div>
      </div>
